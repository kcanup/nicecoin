...
uint256 hashGenesisBlock("0xe27b9eb857555c99cffb648c0998b60149359d066bc54a4dc6e2c5670cd0f320");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // NiceCoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "NiceCoin Signed Message:\n";
...
{
    // NiceCoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    if(nHeight < 5000)      
    {
    nSubsidy = 200 * COIN; // 5000 * 200 = 1000000
    }
    else if(nHeight < 12000)      
    {
    nSubsidy = 120 * COIN; // 7000 * 120 = 840000
    }
    else if(nHeight < 20000) 
    {
    nSubsidy = 80 * COIN; // 8000 * 80 = 640000
    }
     else if(nHeight < 32000) 
    {
    nSubsidy = 50 * COIN; // 12000 * 50 = 600000
    }    
    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 5 * 30; // NiceCoin : diff retargeting every 5 blocks
static const int64 nTargetSpacing = 1 * 30;
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // NiceCoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // NiceCoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
{
    // NiceCoin: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
//1425881551
        hashGenesisBlock = uint256("0xe27b9eb857555c99cffb648c0998b60149359d066bc54a4dc6e2c5670cd0f320");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        // Genesis block
        const char* pszTimestamp = "Fri, 18 May 2015 00:00:00 GMT ==> NiceCoin => Greece-seeks-loan-deal-by-end-of-May";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        block.nVersion = 1;
        block.nTime    = 1431962400;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2060402;
        ...
        assert(block.hashMerkleRoot == uint256("0xec9003fd3fec0ae93841e47c7c2197e6465bfc92eb6ac74edec3a3be4cc1011f"));
...
unsigned char pchMessageStart[4] = { 0xfb, 0xc0, 0xb6, 0xdb }; // NiceCoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("NiceCoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("NiceCoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static NiceCoinMiner(CWallet *pwallet)
{
    printf("NiceCoinMiner:\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("nicecoin-miner");
    ...
    try { loop {
        ...
        printf("Running NiceCoin with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("NiceCoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&NiceCoinMiner, pwallet));
}
